...
from agents import faq_agent, rag_agent, tool_agent, ticket_agent
...
# 8. 分流节点，根据问题类型分配给不同Agent
def dispatch_node(state):
    question = state.get("multi_input", "")
    city = state.get("city")
    express_no = state.get("express_no")
    vectorstore = state.get("vectorstore")
    user_id = state.get("user_id", "user001")
    # 简单意图识别，实际可用LLM或更复杂规则
    if "天气" in question:
        result = tool_agent(question, city=city)
        print(f"[分流] 工具Agent处理: {result}")
        state["dispatch_result"] = result
    elif "快递" in question:
        result = tool_agent(question, express_no=express_no)
        print(f"[分流] 工具Agent处理: {result}")
        state["dispatch_result"] = result
    elif "工单" in question:
        result = ticket_agent(user_id, question)
        print(f"[分流] 工单Agent处理: {result}")
        state["dispatch_result"] = f"已创建工单，编号: {result}"
    else:
        answer = faq_agent(question)
        if answer == "未找到答案":
            print("[分流] FAQ未命中，RAGAgent处理...")
            if vectorstore is None:
                from rag_qa import build_vectorstore
                vectorstore = build_vectorstore()
                state["vectorstore"] = vectorstore
            answer = rag_agent(question, vectorstore)
            state["dispatch_result"] = answer
        else:
            print(f"[分流] FAQAgent处理: {answer}")
            state["dispatch_result"] = answer
    return state
...
def run_customer_service_flow():
    ...
    graph.add_edge("tool", "dispatch")
    graph.add_edge("dispatch", "end")
    graph.add_edge("end", END)

    # 启动流程，支持多轮输入、工具调用和多Agent分流
    memory = None
    multi_inputs = [
        {"input": "请介绍一下智能客服机器人"},
        {"input": "它支持哪些功能？"},
