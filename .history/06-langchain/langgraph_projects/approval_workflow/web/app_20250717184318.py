...
def index(request: Request, tenant: str = "default"):
    ...
    with open(hist_path, 'r', encoding='utf-8') as f:
        history = json.load(f)
    return templates.TemplateResponse("index.html", {"request": request, "requests": requests, "history": history, "tenant": tenant, "tenants": TENANTS})
...
def submit_request(request: Request, user: str = Form(...), amount: float = Form(...), reason: str = Form(...), urgent: bool = Form(False), tenant: str = Form("default")):
    req_path = os.path.join(data_dir, f'requests_{tenant}.json')
    with open(req_path, 'r', encoding='utf-8') as f:
        requests = json.load(f)
...
def edit_flow(request: Request, tenant: str = "default"):
    ...
    with open(flow_path, 'r', encoding='utf-8') as f:
        flow_code = f.read()
    return templates.TemplateResponse("edit_flow.html", {"request": request, "flow_code": flow_code, "tenant": tenant, "tenants": TENANTS})
...
def approve_page(request: Request, req_id: str, user: str):
    # 展示审批表单，支持AI自动意见
    with open(os.path.join(data_dir, 'requests.json'), 'r', encoding='utf-8') as f:
        requests = json.load(f)
    ...
    if req:
        ...
        ai_comment = llm(f"请为如下审批请求生成简明审批意见：用户：{req['user']}，金额：{req['amount']}，事由：{req['reason']}，紧急：{req['urgent']}")
    return templates.TemplateResponse("approve.html", {"request": request, "user": user, "req": req, "ai_comment": ai_comment})

@app.post("/approve/{req_id}", response_class=HTMLResponse)
def do_approve(request: Request, req_id: str, user: str = Form(...), approve: str = Form(...), comment: str = Form(...)):
    # 写入审批历史
    with open(os.path.join(data_dir, 'history.json'), 'r', encoding='utf-8') as f:
        history = json.load(f)
    with open(os.path.join(data_dir, 'requests.json'), 'r', encoding='utf-8') as f:
